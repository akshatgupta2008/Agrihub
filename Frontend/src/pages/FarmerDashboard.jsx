...
import { Video, FileText, CalendarDays, Clock, User, Activity, ChevronRight, Download, Bell, LogOut, TrendingUp, ShieldAlert, Pill, CheckCircle2, XCircle } from "lucide-react";
...
const FarmerDashboard = () => {
  ...
  useEffect(() => {
    const loadSlots = async () => {
      if (!booking.doctorId || !booking.date) { setTimes([]); return; }
      try {
        const res = await axiosInstance.get(`/patient-dashboard/doctors/${booking.doctorId}/slots`, { params: { date: booking.date } });
        setTimes(res.data);
      } catch { setTimes([]); }
    };
    loadSlots();
  ...
  const onBook = async (e) => {
    e.preventDefault();
    if (!booking.doctorId || !booking.date || !booking.time) { toast.error("Select provider, date, and time"); return; }
    setBookingBusy(true);
    try {
  ...
  const patientTypeLabel = authUser?.patientType
    ? authUser.patientType.charAt(0).toUpperCase() + authUser.patientType.slice(1)
    : "General";
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
              <div className="bg-gradient-to-r from-teal-500 to-teal-600 px-6 py-4 flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-white font-semibold text-sm">Request a Booking</h3>
                  <p className="text-teal-100 text-xs">Request a slot with a service provider</p>
                </div>
              </div>
              <div className="p-6">
                <form onSubmit={onBook}>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
                    ...
                    <div>
                      ...
                      >
                        <option value="">
                          {!booking.doctorId || !booking.date ? "Select provider & date first" : times.length ? "Select a slot" : "No slots available"}
                        </option>
                        {times.map((t) => (
                ...
                {loading ? (
                  ...
                  <div className="space-y-3">
                    {appointments.slice(0, appointmentsVisible).map((a) => (
                      <div key={a._id} className="p-3 rounded-xl border border-slate-100 bg-slate-50 hover:bg-teal-50 hover:border-teal-100 transition-all">
                        <div className="flex items-center gap-4">
                          ...
                          <div className="shrink-0 flex flex-col items-end gap-1.5">
                            <div className="flex items-center gap-2">
                              <span className={`text-xs px-2 py-1 rounded-full font-medium ${a.mode === "online" ? "bg-blue-50 text-blue-600 border border-blue-200" : "bg-slate-100 text-slate-600 border border-slate-200"}`}>
            ...
            <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
              <div className="px-5 py-4 border-b border-slate-100 flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-slate-500 text-xs">Most recent farm report</p>
                </div>
              </div>
              <div className="p-5">
                {loading ? (
                  ...
                  <div>
                    <div className="p-3 bg-slate-50 rounded-lg border border-slate-100 mb-3">
                      ...
                      <p className="text-xs text-slate-500 mt-1">{new Date(latestReport.createdAt).toLocaleDateString("en-US", { year: "numeric", month: "long", day: "numeric" })}</p>
                      {latestReport.doctor?.fullName && (
                        <p className="text-xs text-teal-600 mt-0.5">Provider: {latestReport.doctor.fullName}</p>
                      )}
                      {latestReport.notes && (
                        <p className="text-xs text-slate-600 mt-2 line-clamp-2" style={{ whiteSpace: "pre-wrap" }}>{latestReport.notes}</p>
                      )}
                    </div>
                    {(latestReport.fileUrl || latestReport.filePublicId) && (
                      <a
                ...
                {loading ? (
                  ...
                  <div className="space-y-2">
                    {reports.slice(0, 5).map((r) => (
                      <div key={r._id} className="flex items-center gap-3 p-2.5 rounded-lg border border-slate-100 bg-slate-50 hover:bg-teal-50 hover:border-teal-100 transition-all group">
    ...
    <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
      <div className="px-5 py-4 border-b border-slate-100 flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-slate-500 text-xs">Personalized farm insights & recommendations</p>
        </div>
      </div>
      <div className="p-5 space-y-5">
        ...
        <div>
          <p className="text-xs font-semibold text-slate-600 uppercase tracking-wide mb-2 flex items-center gap-1.5">
            <ShieldAlert className="w-3.5 h-3.5 text-slate-400" /> Provider Summary
          </p>
